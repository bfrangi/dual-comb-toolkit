...
temperatures = [t for t in range(1000, 1301, 50)]  # K
...
output_folder = "abs-vs-temp-variation-output"
...
for i, temp in enumerate(temperatures):
    print(
        f"Preparing simulator for temperature {temp} K ({i + 1}/{len(temperatures)})..."
    )
...
# plt.title(
#     f"Peak Absorption vs Temperature for {molecule}\n{vmr} VMR, {pressure} Pa, {length} m path length"
# )
plt.savefig(f"{figures_path}/abs_vs_temp.pdf", dpi=300)
plt.savefig(f"{figures_path}/abs_vs_temp.svg", dpi=300)
plt.clf()
