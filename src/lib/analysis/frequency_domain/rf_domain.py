...
class RadioFrequencyCombExtractor:
    ...
    def comb_frequencies(self) -> list:
        ...
        return [
            float(self.center_freq + (i - n_teeth // 2) * self.freq_spacing)
    ...
    def flip_comb(self) -> None:
        """
        Flip the comb spectrum with respect to the center frequency.
        The index of the center frequency is not changed.
        """
        from numpy import flip, concatenate, array

        self._extracted_amplitude = flip(self.comb_amp)

        if len(self.comb_freq) % 2 == 0:
            self._extracted_frequency = concatenate(
                [self.comb_freq[1::], array([self.comb_freq[-1] + self.freq_spacing])]
            )
    ...
    def generate_spectrum_plot(self) -> "plt":
        ...
        f0 = self.center_freq
        title = (
            "Comb Spectrum showing extracted rf teeth and their amplitudes.\n"
            + f"Central frequency ${f0}$ Hz and repetition rate ${rep}$ Hz."
        )

        return stem_plot(
            self._extracted_frequency,
            self._extracted_amplitude,
            title=title,
            xlabel="Frequency (Hz)",
            ylabel="Amplitude",
        )
